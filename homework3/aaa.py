...
class InfoExtractionGUI:
    def __init__(self, root):
        ...
        self.root.configure(bg="#f0fff0")  # 设置主窗口背景为浅绿色
    ...
    def set_styles(self):
        ...
        # 配置标签框架样式
        style.configure('TLabelframe', background="#ffffff", bordercolor="#c0e0c0", 
                        relief="solid", borderwidth=1)
        style.configure('TLabelframe.Label', background="#ffffff", foreground="#2e8b57")
        
        # 配置标签样式
        style.configure('TLabel', background="#f0fff0", foreground="#2e8b57")
        style.configure('Title.TLabel', font=('微软雅黑', 12, 'bold'), 
                        background="#f0fff0", foreground="#2e8b57")
        
        # 配置按钮样式
        style.configure('TButton', background="#98fb98", foreground="#2e8b57", 
                        font=('微软雅黑', 10), borderwidth=1, relief="solid")
        style.map('TButton', 
                  background=[('active', '#7cfc00'), ('pressed', '#32cd32')],
                  foreground=[('active', '#006400'), ('pressed', '#ffffff')])
        
        # 配置复选框样式
        style.configure('TCheckbutton', background="#f0fff0", foreground="#2e8b57")
        
        # 配置组合框样式
        style.configure('TCombobox', fieldbackground="#ffffff", background="#ffffff", 
                        foreground="#2e8b57", selectbackground="#e0ffe0")
        
        # 配置滚动文本区域样式
        style.configure('TScrolledtext', background="#ffffff", foreground="#2e8b57")
        
        # 配置文本区域样式
        style.configure('TText', background="#ffffff", foreground="#2e8b57")
    ...
    def create_widgets(self):
        ...
        # 标题
        title_label = ttk.Label(main_frame, text="信息抽取系统", 
                               font=('微软雅黑', 16, 'bold'), style='Title.TLabel')
        title_label.grid(row=0, column=0, pady=(0, 15))
        
        # 模式切换区域
        mode_frame = ttk.Frame(main_frame)
        mode_frame.grid(row=1, column=0, sticky=tk.EW, pady=(0, 10))
        
        # Image Mode 按钮
        self.image_mode_btn = ttk.Checkbutton(mode_frame, text="图片模式", 
        ...
        query_frame.grid(row=2, column=0, sticky=tk.EW, pady=(0, 15))
        ...
        # 查询按钮
        self.query_btn = ttk.Button(query_frame, text="查询", command=self.perform_query, style='TButton')
        self.query_btn.grid(row=0, column=4, sticky=tk.W, padx=(10, 0))
    ...
    def create_results_area(self, parent):
        ...
        results_frame.grid(row=3, column=0, sticky=tk.NSEW, pady=(10, 0))
    ...
    def highlight_text_in_widget(self, text_widget, text, keyword):
        ...
        # 配置高亮标签
        text_widget.tag_configure("highlight", background="#98fb98", foreground="#006400")
    ...
    def perform_query(self):
        ...
        stats_label = ttk.Label(self.scrollable_frame, 
                               ...
                               font=('微软雅黑', 11, 'bold'), foreground="#2e8b57")
        stats_label.pack(pady=(0, 15), anchor='w')
    ...
    def create_article_widget(self, parent, article, index):
        ...
        # 完全填充父容器
        article_frame.pack(fill=tk.X, pady=10, padx=0)
        
        # Title部分
        title_label = ttk.Label(article_frame, text="标题:", font=('微软雅黑', 10, 'bold'), foreground="#2e8b57")
        title_label.pack(anchor='w', pady=(0, 5))
        
        title_text = tk.Text(article_frame, height=2, wrap=tk.WORD, font=('微软雅黑', 9), 
                            bg="#ffffff", fg="#2e8b57", relief="solid", borderwidth=1)
        ...
        # Article部分
        article_label = ttk.Label(article_frame, text="内容:", font=('微软雅黑', 10, 'bold'), foreground="#2e8b57")
        article_label.pack(anchor='w', pady=(0, 5))
        
        article_text = scrolledtext.ScrolledText(article_frame, height=6, wrap=tk.WORD, 
                                               font=('微软雅黑', 9), bg="#ffffff", fg="#2e8b57",
                                               relief="solid", borderwidth=1)
        article_content = article.get('article', '')
        article_text.insert('1.0', article_content)
        article_text.config(state='disabled')
        article_text.pack(fill=tk.X, pady=(0, 10))
        ...
        good_label = ttk.Label(rating_frame, text=f"👍 赞: {good_count}", 
                              font=('微软雅黑', 10, 'bold'), foreground="#32cd32")
        good_label.pack(side='left', padx=(0, 20))
        
        bad_label = ttk.Label(rating_frame, text=f"👎 踩: {bad_count}", 
                             font=('微软雅黑', 10, 'bold'), foreground="#ff6347")
        bad_label.pack(side='left', padx=(0, 20))
        
        # 按钮
        good_btn = ttk.Button(rating_frame, text="👍 赞", style='TButton',
                             command=lambda: self.update_article_rating(article['id'], 'Good', good_label))
        good_btn.pack(side='left', padx=(0, 10))
        
        bad_btn = ttk.Button(rating_frame, text="👎 踩", style='TButton',
    ...
    def create_image_widget(self, parent, image_item, index):
        ...
        # 完全填充父容器
        image_frame.pack(fill=tk.X, pady=10, padx=0)
        
        # Title部分
        title_label = ttk.Label(image_frame, text="标题:", font=('微软雅黑', 10, 'bold'), foreground="#2e8b57")
        title_label.pack(anchor='w', pady=(0, 5))
        
        title_text = tk.Text(image_frame, height=2, wrap=tk.WORD, font=('微软雅黑', 9), 
                            bg="#ffffff", fg="#2e8b57", relief="solid", borderwidth=1)
        ...
        # Image部分
        image_label = ttk.Label(image_frame, text="图片:", font=('微软雅黑', 10, 'bold'), foreground="#2e8b57")
    ...
    def update_article_rating(self, article_id, rating_type, label_widget):
        ...
        try:
            ...
            if article_id in self.articles_data:
                ...
                label_widget.config(text=f"👍 赞: {new_count}" if rating_type == 'Good' else f"👎 踩: {new_count}")
